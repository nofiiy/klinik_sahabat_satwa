...
// Validasi visit_id
if (!isset($_GET['id'])) {
    header("Location: kunjungan_vet.php"); // Redirect ke halaman daftar kunjungan jika ID tidak ditemukan
    exit;
}

// Ambil ID visit dari URL
$visit_id = $_GET['id'];

// Query untuk mendapatkan data kunjungan berdasarkan ID visit
$query = "
    SELECT v.visit_id, v.visit_date_time, v.visit_notes, 
           v.animal_id, v.vet_id, v.from_visit_id,
    ...
    WHERE v.visit_id = '$visit_id'";  // Menampilkan data berdasarkan ID visit

$result = mysqli_query($conn, $query);

// Jika tidak ada data yang ditemukan, arahkan ke halaman kunjungan_vet.php
if (mysqli_num_rows($result) == 0) {
    header("Location: kunjungan_vet.php");
    exit;
}

$visit = mysqli_fetch_assoc($result);

// Proses form submission untuk mengupdate data kunjungan
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $from_visit_id = !empty($_POST['from_visit_id']) ? intval($_POST['from_visit_id']) : "NULL"; // Set to NULL if not provided

    // Query untuk mengupdate data kunjungan
    $update_query = "UPDATE visit 
    ...
    if (mysqli_query($conn, $update_query)) {
        header("Location: kunjungan_vet.php");  // Redirect ke halaman daftar kunjungan setelah berhasil update
        exit;
    } else {
...
<head>
    ...
    <title>Edit Visit - Klinik Sahabat Satwa</title>
    <style>
        body {
    ...
    <form method="POST">
        ...
        <select name="animal_id" required>
            ...
            while ($animal = mysqli_fetch_assoc($animals_query)) {
                echo "<option value=\"{$animal['animal_id']}\" " . ($animal['animal_id'] == $visit['animal_id'] ? 'selected' : '') . ">{$animal['animal_id']} - {$animal['animal_name']}</option>";
            }
            ?>
        </select>
        ...
        <select name="vet_id" required>
            ...
            while ($vet = mysqli_fetch_assoc($vets_query)) {
                echo "<option value=\"{$vet['vet_id']}\" " . ($vet['vet_id'] == $visit['vet_id'] ? 'selected' : '') . ">{$vet['vet_id']} - {$vet['vet_givenname']} {$vet['vet_familyname']}</option>";
            }
            ?>
        </select>
